...
export async function POST(req: NextRequest) {
  try {
    ...
    // V2 trennt ab hier strikt:
    //   discoveredCount = wie viele auditierbare Seiten wir finden konnten
    //   auditableUrls   = wie viele davon dieser Tarif tatsächlich tief prüft
    //
    // So kann die UI ehrlich "11 von 47 gefundenen Seiten geprüft" anzeigen,
    // statt bei jedem Gastscan wie eine feste 11-Seiten-Website zu wirken.
    ...
    // "sitemap" if it came from a sitemap, otherwise the page that linked to it.
    ...
    // Homepage-Links ergänzen die Sitemap-Discovery. Das ist wichtig für
    // Websites ohne Sitemap und für Seiten, die in der Sitemap fehlen.
    if (mainHtml) {
      const links = extractInternalLinks(mainHtml, targetUrl).filter(
        (u) =>
          u !== targetUrl &&
          u !== targetUrl.replace(/\/$/, ""),
      );
      ...
      for (const url of links) {
        discoveredSet.add(url);
        if (!sourceMap.has(url)) sourceMap.set(url, targetUrl);
      }
    }

    // Alle gefundenen URLs zuerst klassifizieren. Erst DANACH greift das
    // planabhängige Scan-Limit.
    const allDiscoveredUrls = [...discoveredSet];
    const allAuditableUrls = allDiscoveredUrls.filter(shouldAudit);
    const allFilteredUrls = allDiscoveredUrls.filter((url) => !shouldAudit(url));

    // entdeckteUrls meint Seiten inkl. Startseite. Dadurch ist die Zahl direkt
    // mit "X Seiten geprüft" vergleichbar.
    const discoveredCount = allAuditableUrls.length + 1;
    const filteredCount = allFilteredUrls.length;

    // Nur diese URLs werden tatsächlich tief analysiert.
    const auditableUrls = allAuditableUrls.slice(0, maxSubpages);

    // subpageUrls bleibt für nachgelagerte Logik kompatibel und bezeichnet
    // ab V2 ausschließlich die tatsächlich analysierten Unterseiten.
    ...
    const audit = {
      ...
      nichtGepruefteUrls: Math.max(0, allAuditableUrls.length - auditableUrls.length),
