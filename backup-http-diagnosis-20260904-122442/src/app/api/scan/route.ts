...
export async function POST(req: NextRequest) {
  try {
    ...
    async function verifyInternalLink(linkUrl: string): Promise<VerifiedLink> {
      ...
      const attempt = async (timeoutMs: number): Promise<VerifiedLink> => {
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), timeoutMs);
        ...
        try {
          ...
          const redirected = finalUrl.replace(/\/$/, "") !== linkUrl.replace(/\/$/, "");

          let classification: LinkVerification = "ok";
      ...
      // Ein Timeout/Netzwerkfehler wird einmal mit mehr Zeit verifiziert.
      // Erst danach landet das Ziel als "nicht eindeutig verifizierbar" im Ergebnis.
      const first = await attempt(4500);
      if (first.classification !== "timeout" && first.classification !== "network_error") {
        return first;
      }
    ...
    unterseiten
      .filter((p) => !p.erreichbar)
      .forEach((p) =>
        techIssueLines.push(`- Unterseite nicht erreichbar: ${toPath(p.url)} (${p.status || "keine Antwort"})`)
      );
