...
async function fetchWithTimeout(url: string, timeoutMs = 8000) {
  ...
  try {
    const res = await fetch(url, {
      ...
      redirect: "follow",
      headers: {
        "User-Agent": "WebsiteFix-Scanner/2.0",
...
function classifyHttpStatus(status: number): HttpClassification {
...
/**
 * Gemeinsame HTTP-Verifikation für Unterseiten und interne Links.
 *
 * False-Positive-Schutz:
 * - Timeout/Netzwerkfehler werden einmal mit längerem Timeout wiederholt.
 * - 401/403, 429 und 5xx werden NICHT als "Seite existiert nicht" behandelt.
 * - Redirects werden verfolgt und finalUrl gespeichert.
 */
async function verifyHttpUrl(
  url: string,
  firstTimeoutMs = 5000,
  retryTimeoutMs = 7500,
): Promise<HttpVerification> {
  const attempt = async (timeoutMs: number): Promise<HttpVerification> => {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), timeoutMs);
  ...
  const first = await attempt(firstTimeoutMs);
  if (
    first.classification !== "timeout" &&
    first.classification !== "network_error"
  ) {
    return first;
  }
...
function extractSitemapUrls(xml: string): string[] {
  return extractSitemapLocs(xml).filter((u) => !/\.xml(\?|#|$)/i.test(u));
}


type DiscoveredUrlKind =
  | "page"
  | "feed"
  | "asset"
  | "api"
...
const ASSET_EXTENSIONS =
  /\.(?:jpg|jpeg|png|gif|webp|avif|svg|ico|pdf|zip|rar|7z|mp3|wav|ogg|mp4|webm|mov|avi|css|js|mjs|map|woff2?|ttf|otf|eot|json|xml|txt)(?:\?|#|$)/i;
...
function normalizeCandidateUrl(
  rawUrl: string,
  baseUrl: string,
): { url: string | null; kind: DiscoveredUrlKind; reason: string } {
  try {
    const base = new URL(baseUrl);
    const u = new URL(rawUrl, baseUrl);

    if (u.origin !== base.origin) {
      return { url: null, kind: "external", reason: "andere Domain" };
    }
    ...
    if (ASSET_EXTENSIONS.test(pathname)) {
      return { url: null, kind: "asset", reason: "Asset/Datei" };
    }

    if (FEED_PATTERNS.some((pattern) => pattern.test(pathname))) {
      return { url: null, kind: "feed", reason: "Feed" };
    }

    if (API_PATTERNS.some((pattern) => pattern.test(pathname))) {
      return { url: null, kind: "api", reason: "API-Endpunkt" };
    }

    if (ADMIN_PATTERNS.some((pattern) => pattern.test(pathname))) {
      return { url: null, kind: "admin", reason: "Admin/Login" };
    }

    // WordPress Attachments / typische Medien-Seiten konservativ aussortieren.
    if (
      /\/attachment\//i.test(pathname) ||
      /[?&](?:attachment_id|preview_id)=/i.test(u.search)
    ) {
      return { url: null, kind: "asset", reason: "Attachment/Preview" };
    }

    // Query-Varianten nicht als eigenständige Seiten zählen, wenn die gleiche
    // Pfad-URL bereits ohne relevante Query darstellbar ist.
    // Relevante Pagination bleibt erhalten.
    const allowedParams = new Set(["page", "paged", "p"]);
    ...
    return {
      url: u.href,
      kind: "page",
      reason: "auditierbare HTML-Seite",
    };
  } catch {
    return { url: null, kind: "invalid", reason: "ungültige URL" };
  }
}

function scoreCrawlCandidate(url: string, rootUrl: string): number {
  try {
    ...
    // Wichtige Top-Level-Seiten bevorzugen.
    if (/^\/(?:kontakt|contact)$/i.test(path)) score += 90;
    if (/^\/(?:leistungen|services)$/i.test(path)) score += 85;
    if (/^\/(?:ueber-uns|uber-uns|about)$/i.test(path)) score += 80;
    if (/^\/(?:blog|magazin|news)$/i.test(path)) score += 30;
    if (/^\/(?:impressum|datenschutz|privacy)$/i.test(path)) score -= 50;

    // Root-Domain bevorzugen, www-Unterschied neutral.
    if (
...
function stableSelectUrls(
  ...
  return [...urls]
    .sort((a, b) => {
      const scoreDiff =
        scoreCrawlCandidate(b, rootUrl) - scoreCrawlCandidate(a, rootUrl);
...
/**
 * Liefert eine möglichst ehrliche Zahl auffindbarer Seiten, ohne diese Seiten
 * bereits vollständig zu analysieren.
 *
 * Unterstützt:
 * - /sitemap.xml
 * - /sitemap_index.xml
 * - Sitemap-Index mit Child-Sitemaps
 *
 * Die URL-Liste wird bewusst gedeckelt. Das ist nur Discovery/Scope, kein
 * vollständiger Crawl. Der eigentliche Audit bleibt planabhängig begrenzt.
 */
async function discoverSitemapPages(
  targetUrl: string,
  prefetchedSitemapIndex?: Response | null,
  ...
  async function consumeXml(xml: string) {
    for (const loc of extractSitemapLocs(xml)) {
      if (/\.xml(\?|#|$)/i.test(loc)) {
        if (sitemapDocs.size < MAX_CHILD_SITEMAPS) sitemapDocs.add(loc);
      } else if (shouldAudit(loc) && discovered.size < MAX_DISCOVERED_URLS) {
        try {
          const u = new URL(loc);
          if (u.origin === origin) {
            u.hash = "";
          ...
          // Ungültige Sitemap-URL ignorieren.
        }
      }
    }
  }
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (
        prefetchedSitemapIndex &&
        candidate.endsWith("/sitemap_index.xml")
      ) {
        res = prefetchedSitemapIndex;
      } else {
        res = await fetchWithTimeout(candidate, 5000);
      }
      ...
      await consumeXml(xml);
    } catch {
      // Nächsten Sitemap-Kandidaten probieren.
    }
  }

  const childSitemaps = [...sitemapDocs].slice(0, MAX_CHILD_SITEMAPS);

  const childResults = await batchAsync(
    childSitemaps,
    5,
    async (sitemapUrl) => {
  ...
  // Die Root-URL selbst wird später separat als geprüfte Startseite gezählt.
  const rootNormalized = targetUrl.replace(/\/$/, "");
  discovered.delete(rootNormalized);
  discovered.delete(targetUrl);
...
type PageResult = {
  ...
  erreichbar: boolean;
  status: number;
  availabilityClassification: HttpClassification;
  finalUrl: string;
  redirected: boolean;
  title: string;
  h1: string;
...
async function scanSubpage(url: string, baseUrl: string): Promise<PageResult> {
  const verification = await verifyHttpUrl(url, 5000, 7500);

  // Nur 404/410 und andere eindeutige 4xx-Fehler gelten für das alte
  // kompatible "erreichbar"-Flag als nicht erfolgreich erreichbar.
  // Timeout, 401/403, 429 und 5xx werden separat klassifiziert und dürfen
  // nicht automatisch als "Seite fehlt" in der UI landen.
  const confirmedUnavailable =
    verification.classification === "not_found" ||
    verification.classification === "client_error";

  const empty: PageResult = {
    url,
    erreichbar: !confirmedUnavailable,
    status: verification.status,
    availabilityClassification: verification.classification,
    finalUrl: verification.finalUrl,
    redirected: verification.redirected,
    title: "",
    h1: "",
    metaDescription: "",
    noindex: false,
  ...
  // HTML-/SEO-Prüfungen nur auf erfolgreich abrufbaren Dokumenten.
  if (
    verification.classification !== "ok" ||
  ...
  let html = "";
  try {
    html = await verification.response.text();
  } catch {
    return {
      ...empty,
      availabilityClassification: "network_error",
      status: 0,
    };
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── Development: no guest rate limit on localhost ───────
    const isDevelopment = process.env.NODE_ENV === "development";

    // ── Anonymous users: IP-based rate limit ───────────────
    // In local development we skip this block so repeated test scans are possible.
    // Production keeps the existing burst + IP rate limiting unchanged.
    if (!userId && !hasAdminBypass && !isDevelopment) {
      const now = Date.now();
    ...
    if (!forceRefresh) {
      ...
      if (cached && cacheHasAudit) {
        ...
        type CachedAudit = {
          ...
          altTexte?: { fehlend?: number; eindeutigFehlend?: number; vorkommenFehlend?: number; betroffeneSeiten?: number; gesamt?: number };
          duplicateTitles?: { title: string; seiten: string[] }[];
          duplicateMetas?: { meta: string; seiten: string[] }[];
          brokenLinks?: { url: string; status: number; classification?: string; finalUrl?: string; redirected?: boolean; sourceUrls?: string[] }[];
          linkVerifications?: { url: string; status: number; classification: string; finalUrl?: string; redirected?: boolean; sourceUrls?: string[] }[];
    ...
    // ── 3. DETERMINISTISCHE URL-DISCOVERY ───────────────────
    // Ziel: reproduzierbare, ehrliche Scope-Zahlen.
    // Wir unterscheiden:
    // 1) roh entdeckt
    // 2) als echte HTML-Seite klassifiziert
    // 3) tatsächlich nach Tarif geprüft
    const sitemapDiscovery = await discoverSitemapPages(
      targetUrl,
      sitemapIndexRes,
    );

    scanData.sitemapVorhanden =
      sitemapDiscovery.sitemapFound || !!scanData.sitemapIndexFound;

    const sourceMap = new Map<string, string>();
    const rawCandidates: Array<{ url: string; source: string }> = [];

    for (const url of sitemapDiscovery.urls) {
      rawCandidates.push({ url, source: "sitemap" });
    }

    if (mainHtml) {
      const links = extractInternalLinks(mainHtml, targetUrl);
      for (const url of links) {
        rawCandidates.push({ url, source: targetUrl });
      }
    }

    const discoveryRows: DiscoveredUrl[] = [];
    const canonicalPageMap = new Map<string, { url: string; source: string }>();

    for (const candidate of rawCandidates) {
      const normalized = normalizeCandidateUrl(candidate.url, targetUrl);
      ...
      if (normalized.kind !== "page" || !normalized.url) continue;

      if (!canonicalPageMap.has(normalized.url)) {
        canonicalPageMap.set(normalized.url, {
          url: normalized.url,
          source: candidate.source,
        });
      }
    ...
    const rootNormalized =
      normalizeCandidateUrl(targetUrl, targetUrl).url ??
      targetUrl.replace(/\/$/, "");

    canonicalPageMap.delete(rootNormalized);

    const allPageCandidates = [...canonicalPageMap.values()].map(
      (entry) => entry.url,
    );

    // Stabil priorisieren, damit zwei Scans derselben Website nicht zufällig
    // unterschiedliche 10 Unterseiten auswählen.
    const auditableUrls = stableSelectUrls(
      allPageCandidates,
      targetUrl,
      maxSubpages,
    );
    ...
    for (const url of auditableUrls) {
      sourceMap.set(url, canonicalPageMap.get(url)?.source ?? "discovery");
    }

    const discoveredRawCount = rawCandidates.length;
    const discoveredPageCount = allPageCandidates.length + 1;
    const filteredCount = discoveryRows.filter(
      (row) => row.kind !== "page",
    ).length;
    const duplicateCount =
      discoveryRows.filter((row) => row.kind === "page").length -
      allPageCandidates.length;

    const allFilteredUrls = discoveryRows
      .filter((row) => row.kind !== "page")
      .map((row) => row.rawUrl);

    const subpageUrls = auditableUrls;
    ...
    // V2 trennt "Vorkommen" von "eindeutigen Bildern".
    // Das gleiche Logo kann auf 20 Seiten vorkommen und soll nicht als 20
    // verschiedene Bilder im Ergebnis erscheinen.
    const totalAltMissingOccurrences =
      (scanData.startseite_altMissing as number) +
      unterseiten.reduce((s, p) => s + p.altMissing, 0);

    const totalImages =
      (scanData.startseite_altTotal as number) +
      unterseiten.reduce((s, p) => s + p.altTotal, 0);
    ...
    const uniqueMissingImages = [...new Set(allMissingImageUrls)];
    const totalAltMissing = uniqueMissingImages.length;

    const altAffectedPages =
      ((scanData.startseite_altMissing as number) > 0 ? 1 : 0) +
      unterseiten.filter((p) => p.altMissing > 0).length;

    // ── 8. CHECK 4: BROKEN LINKS ───────────────────────────
    // V2 merkt sich nicht nur das defekte Ziel, sondern auch WO der Link
    // gefunden wurde. Das ist die Basis für eine echte Reparatur-Anleitung.
    const scannedUrls = new Set([
      targetUrl,
      targetUrl.replace(/\/$/, ""),
      ...unterseiten.map((p) => p.url),
    ]);

    const allOutgoing = new Set<string>();
    const linkSources = new Map<string, Set<string>>();

    const registerOutgoing = (sourceUrl: string, links: string[]) => {
      for (const link of links) {
        allOutgoing.add(link);
        const sources = linkSources.get(link) ?? new Set<string>();
    ...
    if (mainHtml) {
      registerOutgoing(targetUrl, extractInternalLinks(mainHtml, targetUrl));
    }
    unterseiten.forEach((p) => registerOutgoing(p.url, p.outgoingLinks));

    const linksToCheck = [...allOutgoing]
      .filter((l) => !scannedUrls.has(l))
      .slice(0, maxBrokenLinkChecks);
    ...
    type VerifiedLink = {
      url: string;
      status: number;
      classification: LinkVerification;
      finalUrl: string;
      redirected: boolean;
      sourceUrls: string[];
    };

    async function verifyInternalLink(linkUrl: string): Promise<VerifiedLink> {
      const sourceUrls = [...(linkSources.get(linkUrl) ?? [])];
      const result = await verifyHttpUrl(linkUrl, 4500, 7000);

      const classification: LinkVerification =
    ...
    // Verifikation bewusst in kleinen Batches: weniger False Positives durch
    // Bot-/Rate-Limits des Zielservers und weniger Last auf fremden Websites.
    ...
    // Nur echte 4xx-Fehler werden als "defekte Links" gezählt.
    // Timeout, 5xx, 401/403 oder Rate-Limits sind separate Diagnosezustände.
    const brokenLinks = brokenLinkResults.filter(
      (result) => result.classification === "broken",
    );

    const unverifiedLinks = brokenLinkResults.filter(
      (result) =>
    ...
    const orphanedPages = auditableUrls.filter((u) => {
    ...
    const audit = {
      ...
      // Legacy-Feld für bestehende Clients. Semantisch ist dies ab V2
      // eine Discovery-Kandidatenzahl, KEINE bestätigte Seitenzahl.
      entdeckteUrls: discoveredPageCount,
      discoveryRawCount: discoveredRawCount,
      discoveryCandidateCount: discoveredPageCount,
      verifiedPagesCount: unterseiten.length + 1,
      discoveryPageCount: discoveredPageCount,
      discoveryFilteredCount: filteredCount,
      discoveryDuplicateCount: Math.max(0, duplicateCount),
      discoveryBreakdown: {
        page: discoveryRows.filter((row) => row.kind === "page").length,
        feed: discoveryRows.filter((row) => row.kind === "feed").length,
        asset: discoveryRows.filter((row) => row.kind === "asset").length,
        api: discoveryRows.filter((row) => row.kind === "api").length,
        admin: discoveryRows.filter((row) => row.kind === "admin").length,
        external: discoveryRows.filter((row) => row.kind === "external").length,
        invalid: discoveryRows.filter((row) => row.kind === "invalid").length,
      },
      gefilterteUrls: filteredCount,
      uebersprungeneUrls: allFilteredUrls.slice(0, 100),
      scanLimitUnterseiten: maxSubpages,
      nichtGepruefteUrls: Math.max(0, allPageCandidates.length - auditableUrls.length),
      unterseiten: unterseiten.map((p) => ({
        url: p.url,
        erreichbar: p.erreichbar,
        status: p.status,
        availabilityClassification: p.availabilityClassification,
        finalUrl: p.finalUrl,
      ...
      pageVerifications: unterseiten
        .filter((p) => p.availabilityClassification !== "ok")
        .map((p) => ({
          url: p.url,
          status: p.status,
          classification: p.availabilityClassification,
          finalUrl: p.finalUrl,
          redirected: p.redirected,
          foundVia: p.foundVia,
        })),
      ...
      duplicateMetas,
      altTexte: {
        // "fehlend" bleibt aus Kompatibilitätsgründen bestehen, meint ab V2 aber
        // eindeutige Bild-URLs statt mehrfach gezählter Seiten-Vorkommen.
        fehlend: totalAltMissing,
        eindeutigFehlend: totalAltMissing,
        vorkommenFehlend: totalAltMissingOccurrences,
        betroffeneSeiten: altAffectedPages,
        gesamt: totalImages,
        missingImages: uniqueMissingImages.slice(0, 10),
      },
      ...
      linkVerifications: unverifiedLinks.map((b) => ({
        url: b.url,
        status: b.status,
        classification: b.classification,
        finalUrl: b.finalUrl,
    ...
    // ── 10. DETERMINISTISCHE EXTERNE DIAGNOSE ────────────────
    // V2: Der kostenlose Außen-Scan wartet nicht mehr auf ein LLM.
    // Scanner-Fakten und Priorisierung sind deterministisch. KI gehört später
    // in die vertiefte Erklärung/Fix-Anleitung, nicht in die Messung selbst.
    ...
    const fc = scanData.formCheck as {
      formsCount: number;
      inputsWithoutLabel: number;
      buttonsWithoutText: number;
    } | undefined;
    ...
    if (!scanData.h1) techIssueLines.push("- Startseite: H1 fehlt");
    if (!scanData.https) techIssueLines.push("- Startseite: HTTPS fehlt");
    if (scanData.indexierungGesperrt) techIssueLines.push("- Startseite: noindex gesetzt");
    if (!scanData.sitemapVorhanden) techIssueLines.push("- Sitemap wurde nicht gefunden");
    if (scanData.robotsBlockiertAlles) techIssueLines.push("- robots.txt blockiert allgemeine Crawler");

    if (brokenLinks.length > 0) {
      techIssueLines.push(`- ${brokenLinks.length} interne Linkziele liefern keine erfolgreiche Antwort`);
    }

    unterseiten
      .filter(
        (p) =>
          p.availabilityClassification === "not_found" ||
          p.availabilityClassification === "client_error",
      )
      .forEach((p) =>
        techIssueLines.push(
          `- Unterseite mit bestätigtem HTTP-Fehler: ${toPath(p.url)} (HTTP ${p.status})`,
        ),
      );
    ...
    unterseiten
      .filter(
        (p) =>
          p.availabilityClassification === "timeout" ||
          p.availabilityClassification === "network_error" ||
          p.availabilityClassification === "protected" ||
          p.availabilityClassification === "rate_limited" ||
          p.availabilityClassification === "server_error",
      )
      .forEach((p) =>
        techIssueLines.push(
          `- Unterseite nicht eindeutig verifizierbar: ${toPath(p.url)} (${p.availabilityClassification}${p.status ? ` / HTTP ${p.status}` : ""})`,
        ),
      );
    ...
    unterseiten
      .filter((p) => p.noindex)
      .forEach((p) =>
        techIssueLines.push(`- Unterseite mit noindex: ${toPath(p.url)}`)
      );

    unterseiten
      .filter((p) => !p.title || p.title === "(kein Title)")
      .forEach((p) =>
        techIssueLines.push(`- Unterseite ohne Title: ${toPath(p.url)}`)
      );

    if (totalAltMissing > 0) {
      techIssueLines.push(
        `- ${totalAltMissing} eindeutige Bild-URLs ohne Alt-Text (${totalAltMissingOccurrences} Vorkommen auf ${altAffectedPages} Seiten)`
      );
    }

    if ((fc?.inputsWithoutLabel ?? 0) > 0) {
      techIssueLines.push(`- ${fc!.inputsWithoutLabel} Formularfelder ohne erkennbare Beschriftung`);
    }

    if (duplicateTitles.length > 0) {
      techIssueLines.push(`- ${duplicateTitles.length} Gruppen mit doppelten Title-Tags`);
    }

    if (duplicateMetas.length > 0) {
      techIssueLines.push(`- ${duplicateMetas.length} Gruppen mit doppelten Meta-Descriptions`);
    }

    const diagnose =
      techIssueLines.length > 0
        ? `Technische Befunde für ${host0}:\n${techIssueLines.join("\n")}`
        : `Bei den geprüften externen Signalen wurden für ${host0} keine akuten technischen Auffälligkeiten erkannt.`;
