...
export function buildDiagnosis(
  ...
  if (
    ...
    const hasConfirmedHttpEvidence = confirmedUnavailableRaw.length > 0;

    findings.push({
      key: "availability",
      priority: "critical",
      title: hasConfirmedHttpEvidence
        ? "Seiten mit bestätigtem HTTP-Fehler"
        : "Seiten nicht erfolgreich erreichbar",
      summary: hasConfirmedHttpEvidence
        ? `${affectedUrls.length} ${
        ...
        : `${affectedUrls.length} ${
            affectedUrls.length === 1
              ? "Seite war"
              : "Seiten waren"
          } im gespeicherten Scan nicht erfolgreich erreichbar. Für einen bestätigten HTTP-Status ist ein frischer V2-Scan erforderlich.`,
      evidence,
      affectedUrls,
      possibleCauses: [
        has404 || has410
