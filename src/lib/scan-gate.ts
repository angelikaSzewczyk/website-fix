 ...
 * - In `npm run dev` ist dieses Client-Gate komplett deaktiviert.
 * - Es wird lokal weder geblockt noch ein Scan-/Rate-Limit-Eintrag gespeichert.
 * - In Production bleibt das bestehende Verhalten unverändert aktiv.
 *
 * Server-side Rate-Limiting in /api/scan bleibt die echte Anti-Abuse-Schicht.
 */
...
function readGate(): GateRecord | null {
  // In Development keinerlei lokale Gate-Einträge auswerten.
...
export function isScanBlocked(
  url?: string
): { blocked: boolean; nextMs: number; sameUrl: boolean } {
  // Development = immer freigeben.
  if (isDevelopment) {
    return {
      blocked: false,
      nextMs: 0,
...
export function recordScan(
  ...
  // Development: nichts in localStorage schreiben.
...
export function recordRateLimit(
  ...
  // Development: kein lokales Rate-Limit persistieren.
  if (isDevelopment) return;

  recordScan(
    url,
    nextMs - FREE_SCAN_LIMIT_MS
  );
}
