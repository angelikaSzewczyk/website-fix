...
import PlanStatus from "@/components/results/PlanStatus";
...
function ResultsInner() {
  ...
  useEffect(() => {
    ...
    // 1) Frischer Scan: volle Evidence direkt aus derselben Browser-Session.
    try {
      const raw = sessionStorage.getItem("wf_scan_raw_evidence");
      if (raw) {
        const parsed = JSON.parse(raw) as {
          url?: string;
          scanData?: RawScanEvidence | null;
          savedAt?: number;
        };
        ...
        if (
          parsed.url &&
          parsed.scanData &&
          normalizeUrl(parsed.url) === normalizeUrl(urlParam) &&
          (!parsed.savedAt || Date.now() - parsed.savedAt < 2 * 60 * 60 * 1000)
        ) {
          setRawEvidence(parsed.scanData);
          return;
        }
      }
    } catch {
    ...
    // 2) Neue Tabs / Reload: Cache-Fallback.
    fetch(`/api/scan/cached?url=${encodeURIComponent(urlParam)}`)
      .then((response) => response.json())
      ...
      .catch(() => {
        // Results bleiben mit dem kompakten StoredScan nutzbar.
      });
  }, [urlParam]);
