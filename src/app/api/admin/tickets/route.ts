 ...
 * POST /api/admin/tickets       — reply to / close a ticket
 */
...
export async function POST(req: NextRequest) {
  ...
  if (body.action === "close") {
    await sql`UPDATE support_tickets SET status = 'closed' WHERE id::text = ${body.ticketId}`;
    return NextResponse.json({ ok: true });
  }

  if (body.action === "reopen") {
    await sql`UPDATE support_tickets SET status = 'open', admin_reply = NULL, replied_at = NULL WHERE id::text = ${body.ticketId}`;
    return NextResponse.json({ ok: true });
  }
